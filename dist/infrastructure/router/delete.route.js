...
router.delete("/:name", log_1.logMiddleware, (req, res, _next) => {
    const file = path_1.default.join(__dirname, '../../storage/media/' + req.params.name);
    fs_1.default.exists(file, function (exists) {
        if (!exists) {
            ...
            const storageCodeDir = fs_1.default.readdirSync(path_1.default.join(__dirname, '../../storage/media/'));
            const data = {
                err: false,
                status: "200",
